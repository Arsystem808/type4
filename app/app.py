...
# --- Import guard: добавить корень проекта в PYTHONPATH ---
...
# Безопасные дефолты (чтобы не ловить NameError при первом рендере)
go = False
dev_mode = False
show_gauge = True
...
"""
    ...
    # --- Правая колонка: индикатор, JSON (dev), бэктест ---
    with col2:
        # Индикатор «Общая оценка»: преобразуем действие+уверенность в шкалу [-2..+2]
        score = 0.0
        if sig.action == "BUY":
        ...
        # JSON — только в режиме разработчика
        if dev_mode:
            st.markdown("#### JSON")
            st.code(
