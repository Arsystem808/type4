...
def render_gauge_svg(
    ...
    max_width: int = 660,       # «потолок» ширины
    scale: float = 0.85,        # ↓ общий размер прибора (0.70–1.00)
    font_scale: float = 0.88,   # ↓ шрифты (0.70–1.10)
    dark_bg: str = "#0E1117",
    animate: bool = True,
    duration_ms: int = 900,
) -> str:
    ...
    # Геометрия (компактнее)
    W = int(max_width * scale)
    H = int(W * 0.60)
    cx, cy, R = W/2, H*0.87, W*0.40  # радиус чуть меньше
    arc_thick   = max(2, int(W * 0.036))  # тоньше дуга
    outline_w   = 2
    tick_in, tick_out, tick_txt = R - 10, R + 3, R + 38
    ...
    # Засечки и числа (вернули 0)
    ticks = [(-180, "−2"), (-135, "−1"), (-90, "0"), (-45, "+1"), (0, "+2")]
    tick_lines, tick_texts = [], []
    for a, lab in ticks:
        ax1, ay1 = cx + tick_in * math.cos(math.radians(a)), cy + tick_in * math.sin(math.radians(a))
        ax2, ay2 = cx + tick_out* math.cos(math.radians(a)), cy + tick_out* math.sin(math.radians(a))
        tx,  ty  = cx + tick_txt* math.cos(math.radians(a)), cy + tick_txt* math.sin(math.radians(a))
        tick_lines.append(
            f'<line x1="{ax1:.1f}" y1="{ay1:.1f}" x2="{ax2:.1f}" y2="{ay2:.1f}" stroke="#FFFFFF" stroke-opacity="0.9" stroke-width="1.4" />'
        )
        tick_texts.append(
            f'<text x="{tx:.1f}" y="{ty:.1f}" text-anchor="middle" dominant-baseline="middle" class="t tick halo">{lab}</text>'
        )

    # Шрифты (процент от ширины * font_scale)
    ...
    needle_len = R - 8
    needle_w   = max(2, int(W * 0.012))   # тоньше стрелка
    hub_r      = max(2, int(W * 0.010))

    needle = f"""
    <g transform="rotate({start_ang:.2f} {cx:.1f} {cy:.1f})">
      <line x1="{cx:.1f}" y1="{cy:.1f}" x2="{cx + needle_len:.1f}" y2="{cy:.1f}"
            stroke="#FFFFFF" stroke-width="{needle_w}" stroke-linecap="round" />
      <circle cx="{cx:.1f}" cy="{cy:.1f}" r="{hub_r}" fill="#FFFFFF"/>
...
<div style="max-width:{W}px;width:100%;margin:0 auto;">
  <svg viewBox="0 0 {W} {H}" width="100%" height="auto" preserveAspectRatio="xMidYMid meet"
    ...
    <!-- Дуга и обводка -->
    <path d="{arc}"     stroke="url(#grad)" stroke-width="{arc_thick}" stroke-linecap="round" fill="none"/>
    <path d="{outline}" stroke="#FFFFFF"    stroke-opacity="0.92" stroke-width="{outline_w}" fill="none"/>
